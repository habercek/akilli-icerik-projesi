...
// --- YARDIMCI BİLEŞEN: Optimizasyon Sonuçlarını Göstermek İçin ---
function OptimizationResults({ data }) {
    ...
    const renderSubheadings = () => {
        if (Array.isArray(data.subheadings)) {
            return (
                <ul>
                    {data.subheadings.map((sh, index) => <li key={index}>{sh}</li>)}
                </ul>
            );
        }
        return <p>{data.subheadings}</p>; // Eğer string gelirse diye fallback
    };
    ...
    return (
        <div className="results-container">
            ...
            <div className="result-item">
                <strong>H1 Başlığı:</strong>
                <p>{data.h1_title}</p>
            </div>
            <div className="result-item">
                <strong>Meta Açıklaması:</strong>
            ...
            <div className="result-item">
                <strong>Alt Başlık Önerileri:</strong>
                {renderSubheadings()}
            </div>
        </div>
    );
...
// --- ANA DÜZENLEME SAYFASI BİLEŞENİ ---
function EditArticlePage({ articleData }) {
    ...
    const handleOptimize = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(result.error || 'Bilinmeyen bir optimizasyon hatası oluştu.');
            }
            ...
            setOptimizeMessage(`Hata: ${err.message}`);
        } finally {
        ...
        <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
             <style jsx global>{`
                .article-preview img {
                    ...
                    margin: 0 auto 1rem auto !important; max-width: 100%; height: auto;
                }
                .results-container {
                    margin-top: 25px; padding: 20px;
                    border: 1px solid #1abc9c; border-radius: 8px; background-color: #f0fdfa;
                }
                .result-item { margin-bottom: 15px; }
                .result-item strong { display: block; margin-bottom: 5px; color: #16a085; }
                .result-item p { margin: 0; }
                .result-item ul { margin: 0; padding-left: 20px; }
            `}</style>
            ...
            <div>
                <form onSubmit={handleSave}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                        <button type="button" onClick={() => setViewMode('preview')} disabled={viewMode === 'preview'} style={{ padding: '8px 12px', cursor: 'pointer', border: '1px solid #ccc', backgroundColor: viewMode === 'preview' ? '#007bff' : 'white', color: viewMode === 'preview' ? 'white' : 'black' }}>
                            Canlı Önizleme
                        </button>
                        <button type="button" onClick={() => setViewMode('html')} disabled={viewMode === 'html'} style={{ marginLeft: '0.5rem', padding: '8px 12px', cursor: 'pointer', border: '1px solid #ccc', backgroundColor: viewMode === 'html' ? '#007bff' : 'white', color: viewMode === 'html' ? 'white' : 'black' }}>
                            HTML Kodunu Düzenle
                        </button>
                    </div>
                    {viewMode === 'preview' ? (
                    ...
                    <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button type="button" onClick={handleOptimize} disabled={isOptimizing || isSaving} style={{ padding: '10px 20px', fontSize: '16px', backgroundColor: '#6f42c1', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                            {isOptimizing ? 'Optimize Ediliyor...' : 'Yapay Zeka ile Optimize Et →'}
                        </button>
                    </div>
                ...
                {/* Optimizasyon Mesajı ve Sonuçları */}
...
export async function getServerSideProps(context) {
    try {
        ...
        // Sayfa yüklendiğinde var olan SEO verisini de alıp yolluyoruz
        const articleData = {
            id: docSnap.id,
